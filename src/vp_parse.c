/*
** vp_parse.c
** Syntax analyser
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "vp_parse.h"
#include "vp_ast.h"
#include "vp_def.h"
#include "vp_lex.h"
#include "vp_state.h"
#include "vp_str.h"
#include "vp_type.h"
#include "vp_vec.h"

/* Precedence levels for operators */
typedef enum
{
    PREC_NONE,
    PREC_ASSIGN,     /*  =  */
    PREC_OR,         /*  or  */
    PREC_AND,        /*  and  */
    PREC_EQUALITY,   /*  == !=  */
    PREC_COMPARISON, /*  < > <= >=  */
    PREC_BOR,        /* | */
    PREC_BXOR,       /* ^ */
    PREC_BAND,       /* & */
    PREC_SHIFT,      /* << >> */
    PREC_TERM,       /*  + -  */
    PREC_FACTOR,     /*  * /  */
    PREC_UNARY,      /*  not ! - ~  */
    PREC_CALL,       /*  () */
} Prec;

typedef Expr* (*ParsePrefixFn)(LexState* ls); 
typedef Expr* (*ParseInfixFn)(LexState* ls, Expr* lhs);

typedef struct
{
    ParsePrefixFn prefix;
    ParseInfixFn infix;
    Prec prec;
} ParseRule;

void vp_parse_error(SrcPos sp, const char* msg, ...)
{
    va_list args;
    va_start(args, msg);
    fprintf(stderr, "%s:%d ", sp.name, sp.line);
    vfprintf(stderr, msg, args);
    fputc('\n', stderr);
    va_end(args);
    exit(EXIT_FAILURE);
}

/* Lexical source position */
#define lex_srcpos(ls) ((SrcPos){.line = (ls)->linenumber, .name = (ls)->name})

/* Check for matching token and consume it */
static void lex_consume(LexState* ls, LexToken t)
{
    if(ls->curr == t)
    {
        vp_lex_next(ls);
        return;
    }
    const char* tokstr = vp_lex_tok2str(ls, t);
    const char* prev = vp_lex_tok2str(ls, ls->curr);
    vp_lex_error(ls, 0, "Expected %s, got %s\n", tokstr, prev);
}

/* Check for matching token */
static bool lex_check(LexState* ls, LexToken t)
{
    return ls->curr == t;
}

/* Check and consume token */
static bool lex_match(LexState* ls, LexToken t)
{
    if(ls->curr != t)
        return false;
    vp_lex_next(ls);
    return true;
}

/* Forward declarations */
static Decl* parse_decl(LexState* ls);
static ParseRule expr_rule(LexToken t);
static Expr* expr_prec(LexState* ls, Prec prec);
static Expr* expr(LexState* ls);

/* Parse literal expression */
static Expr* expr_lit(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    switch(ls->prev)
    {
        case TK_true:
            return vp_expr_true(sp);
        case TK_false:
            return vp_expr_false(sp);
        case TK_nil:
            return vp_expr_nil(sp);
        case TK_integer:
            return vp_expr_ilit(sp, ls->val.i);
        case TK_number:
            return vp_expr_flit(sp, ls->val.n);
        case TK_string:
            return vp_expr_str(sp, ls->val.name);
        default:
            vp_assertX(false, "Unknown literal");
            break;
    }
    return NULL;    /* unreachable */
}

/* Parse grouping expression */
static Expr* expr_group(LexState* ls)
{
    Expr* e = expr(ls);
    lex_consume(ls, ')');
    return e;
}

/* Forward declaration */
static TypeSpec* parse_type(LexState* ls);

/* Parse cast expression */
static Expr* expr_cast(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    lex_consume(ls, '(');
    TypeSpec* spec = parse_type(ls);
    lex_consume(ls, ',');
    Expr* e = expr(ls);
    lex_consume(ls, ')');
    return vp_expr_cast(sp, spec, e);
}

/* Parse call expression */
static Expr* expr_call(LexState* ls, Expr* lhs)
{
    SrcPos sp = lex_srcpos(ls);
    Expr** args = NULL;
    if(!lex_check(ls, ')'))
    {
        do
        {
            Expr* e = expr(ls);
            vec_push(args, e);
        }
        while(lex_match(ls, ','));
    }
    lex_consume(ls, ')');
    return vp_expr_call(sp, lhs, args);
}

/* Parse index subscript expression */
static Expr* expr_idx(LexState* ls, Expr* lhs)
{
    SrcPos sp = lex_srcpos(ls);
    Expr* idx = expr(ls);
    lex_consume(ls, ']');
    return vp_expr_idx(sp, lhs, idx);
}

/* Parse named field expression */
static Expr* expr_dot(LexState* ls, Expr* lhs)
{
    SrcPos sp = lex_srcpos(ls);
    lex_consume(ls, TK_name);
    Str* name = ls->val.name;
    return vp_expr_field(sp, lhs, name);
}

/* Parse unary expression */
static Expr* expr_unary(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    LexToken tok = ls->prev;
    Expr* expr = expr_prec(ls, PREC_UNARY);
    ExprKind kind = 0;
    switch(tok)
    {
        case '-': kind = EX_NEG; break;
        case '~': kind = EX_BNOT; break;
        case '!':
        case TK_not:
            kind = EX_NOT;
            break;
        default:
            vp_assertX(false, "Unknown unary op");
            break;
    }
    return vp_expr_unary(sp, kind, expr);
}

/* Parse binary expression */
static Expr* expr_binary(LexState* ls, Expr* lhs)
{
    SrcPos sp = lex_srcpos(ls);
    LexToken tok = ls->prev;
    ParseRule rule = expr_rule(tok);
    Expr* rhs = expr_prec(ls, (Prec)(rule.prec + 1));
    ExprKind kind = 0;
    switch(tok)
    {
        case '+': kind = EX_ADD; break;
        case '-': kind = EX_SUB; break;
        case '*': kind = EX_MUL; break;
        case '/': kind = EX_DIV; break;
        case '%': kind = EX_MOD; break;
        case '&': kind = EX_BAND; break;
        case '|': kind = EX_BOR; break;
        case '^': kind = EX_BXOR; break;
        case TK_lshift: kind = EX_LSHIFT; break;
        case TK_rshift: kind = EX_RSHIFT; break;
        case TK_eq: kind = EX_EQ; break;
        case TK_noteq: kind = EX_NOTEQ; break;
        case '<': kind = EX_LT; break;
        case TK_le: kind = EX_LE; break;
        case '>': kind = EX_GT; break;
        case TK_ge: kind = EX_GE; break;
        case TK_and: kind = EX_AND; break;
        case TK_or: kind = EX_OR; break;
        default:
            vp_assertX(false, "Unknown binary op");
            break;  /* Unreachable */
    }
    return vp_expr_binop(sp, kind, lhs, rhs);
}

/* Parse name expression */
static Expr* expr_name(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    Str* name = ls->val.name;
    return vp_expr_name(sp, name);
}

static Field expr_field(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    if(lex_match(ls, '['))
    {
        Expr* idx = expr(ls);
        lex_consume(ls, ']');
        lex_consume(ls, '=');
        Expr* e = expr(ls);
        return (Field){.sp = sp, .kind = FIELD_IDX, .init = e, .idx = idx};
    }
    else
    {
        Expr* e = expr(ls);
        if(lex_match(ls, '='))
        {
            if(e->kind != EX_NAME)
                vp_parse_error(e->sp, "Expected field name");
            return (Field){.sp = sp, .kind = FIELD_NAME, .init = e, .name = e->name};
        }
        else
        {
            return (Field){.sp = sp, .kind = FIELD_DEFAULT, .init = e};
        }
    }
}

/* Parse compound literal expression */
static Expr* expr_comp(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    Field* fields = NULL;
    while(!lex_check(ls, '}'))
    {
        Field c = expr_field(ls);
        vec_push(fields, c);
        if(!lex_match(ls, ','))
            break;
    }
    lex_consume(ls, '}');
    return vp_expr_comp(sp, fields);
}

#define NONE                    (ParseRule){ NULL, NULL, PREC_NONE }
#define RULE(pr, in, prec)      (ParseRule){ pr, in, prec }
#define INFIX(in)               (ParseRule){ NULL, in, PREC_NONE }
#define PREFIX(pr)              (ParseRule){ pr, NULL, PREC_NONE }
#define OPERATOR(in, prec)      (ParseRule){ NULL, in, prec }

/* Find the rule of a given token */
static ParseRule expr_rule(LexToken t)
{
    switch(t)
    {
        /* Literals */
        case TK_true:
        case TK_false:
        case TK_nil:
        case TK_integer:
        case TK_number:
        case TK_string:
            return PREFIX(expr_lit);
        case TK_name:
            return PREFIX(expr_name);
        case '{':
            return PREFIX(expr_comp);
        case '(':
            return RULE(expr_group, expr_call, PREC_CALL);
        case '[':
            return RULE(NULL, expr_idx, PREC_CALL);
        case '.':
            return RULE(NULL, expr_dot, PREC_CALL);
        case TK_cast:
            return PREFIX(expr_cast);
        /* Unary */
        case TK_not:
        case '!':
        case '~':
            return PREFIX(expr_unary);
        /* Binary */
        case '+':
            return OPERATOR(expr_binary, PREC_TERM);
        case '-':
            return RULE(expr_unary, expr_binary, PREC_TERM);
        case '*':
        case '/':
            return OPERATOR(expr_binary, PREC_FACTOR);
        case '%':
            return OPERATOR(expr_binary, PREC_FACTOR);
        case '&':
            return OPERATOR(expr_binary, PREC_BAND);
        case '|':
            return OPERATOR(expr_binary, PREC_BOR);
        case '^':
            return OPERATOR(expr_binary, PREC_BXOR);
        case TK_lshift:
        case TK_rshift:
            return OPERATOR(expr_binary, PREC_SHIFT);
        case TK_noteq:
        case TK_eq:
            return OPERATOR(expr_binary, PREC_EQUALITY);
        case '<':
        case '>':
        case TK_ge:
        case TK_le:
            return OPERATOR(expr_binary, PREC_COMPARISON);
        case TK_and:
            return OPERATOR(expr_binary, PREC_AND);
        case TK_or:
            return OPERATOR(expr_binary, PREC_OR);
        default:
            return NONE;
    }
}

#undef NONE
#undef RULE
#undef INFIX
#undef PREFIX
#undef OPERATOR

/* Parse precedence */
static Expr* expr_prec(LexState* ls, Prec prec)
{
    vp_lex_next(ls);
    SrcPos sp = lex_srcpos(ls);
    ParsePrefixFn prefix = expr_rule(ls->prev).prefix;
    if(prefix == NULL)
    {
        vp_parse_error(sp, "Expected expression");
    }

    Expr* expr = prefix(ls);
    while(prec <= expr_rule(ls->curr).prec)
    {
        vp_lex_next(ls);
        ParseInfixFn infix = expr_rule(ls->prev).infix;
        expr = infix(ls, expr);
    }
    return expr;
}

/* Parse expression */
static Expr* expr(LexState* ls)
{
    return expr_prec(ls, PREC_ASSIGN);
}

static Type* parse_type_builtin(LexToken tok)
{
    Type* ty = NULL;
    switch(tok)
    {
    case TK_void: ty = tyvoid; break;
    case TK_bool: ty = tybool; break;
    case TK_uint8: ty = tyuint8; break;
    case TK_uint16: ty = tyuint16; break;
    case TK_uint32: ty = tyuint32; break;
    case TK_uint64: ty = tyuint64; break;
    case TK_int8: ty = tyint8; break;
    case TK_int16: ty = tyint16; break;
    case TK_int32: ty = tyint32; break;
    case TK_int64: ty = tyint64; break;
    case TK_float: ty = tyfloat; break;
    case TK_double: ty = tydouble; break;
    default: break;
    }
    return ty;
}

static TypeSpec* parse_type_fn(LexState* ls)
{
    SrcPos sp = lex_srcpos(ls);
    TypeSpec** args = NULL;
    lex_consume(ls, '(');
    if(!lex_check(ls, ')'))
    {
        do
        {
            TypeSpec* spec = parse_type(ls);
            vec_push(args, spec);
        }
        while(lex_match(ls, ','));
    }
    lex_consume(ls, ')');
    lex_consume(ls, ':');
    TypeSpec* ret = parse_type(ls);
    return vp_typespec_fn(sp, ret, args);
}

/* Parse type information (keyword or name) */
static TypeSpec* parse_type(LexState* ls)
{
    TypeSpec* spec = NULL;
    if(lex_match(ls, TK_name))
    {
        SrcPos sp = lex_srcpos(ls);
        Str* name = ls->val.name;
        spec = vp_typespec_name(sp, name);
    }
    else if(lex_match(ls, TK_fn))
    {
        spec = parse_type_fn(ls);
    }
    else
    {
        SrcPos sp = lex_srcpos(ls);
        Type* type = parse_type_builtin(ls->curr);
        if(type == NULL)
        {
            vp_parse_error(sp, "Unexpected token type");
        } 
        vp_lex_next(ls);    /* Skip type */
        spec = vp_typespec_type(sp, type);
    }
    SrcPos sp = lex_srcpos(ls);
    while(lex_check(ls, '*') || lex_check(ls, '['))
    {
        if(lex_match(ls, '['))
        {
            Expr* size = NULL;
            if(!lex_check(ls, ']')) size = expr(ls);
            lex_consume(ls, ']');
            spec = vp_typespec_array(sp, spec, size);
        }
        else
        {
            lex_consume(ls, '*');
            spec = vp_typespec_ptr(sp, spec);
        }
    }
    return spec;
}

/* Forward declaration */
static Stmt* parse_stmt(LexState* ls);

static Str* parse_name(LexState* ls)
{
    lex_consume(ls, TK_name);
    return ls->val.name;
}

/* Parse code block {} */
static Stmt* parse_block(LexState* ls)
{
    Stmt** stmts = NULL;
    lex_consume(ls, '{');
    SrcPos sp = lex_srcpos(ls);
    while(!lex_check(ls, '}') && !lex_check(ls, TK_eof))
    {
        Stmt* st = NULL;
        Decl* d = parse_decl(ls);
        if(d)
        {
            st = vp_stmt_decl(d->sp, d);
        }
        else
        {
            st = parse_stmt(ls);
        }
        vec_push(stmts, st);
    }
    lex_consume(ls, '}');
    return vp_stmt_block(sp, stmts);
}

/* Parse 'return' statement */
static Stmt* parse_return(LexState* ls)
{
    vp_lex_next(ls);    /* Skip 'return' */
    SrcPos sp = lex_srcpos(ls);
    Expr* e = expr(ls);
    lex_consume(ls, ';');
    return vp_stmt_return(sp, e);
}

/* Parse fn parameters */
static Param* parse_params(LexState* ls)
{
    Param* params = NULL;
    lex_consume(ls, '(');
    if(!lex_check(ls, ')'))
    {
        do
        {
            SrcPos sp = lex_srcpos(ls);
            Str* name = parse_name(ls);
            lex_consume(ls, ':');
            TypeSpec* spec = parse_type(ls);
            Param p = (Param){.sp = sp, .name = name, .spec = spec};
            vec_push(params, p);
        }
        while(lex_match(ls, ','));
    }
    lex_consume(ls, ')');
    return params;
}

static Decl* parse_typedef(LexState* ls)
{
    vp_lex_next(ls);    /* Skip 'type' */
    SrcPos sp = lex_srcpos(ls);
    lex_consume(ls, TK_name);
    Str* name = ls->val.name;
    lex_consume(ls, '=');
    TypeSpec* spec = parse_type(ls);
    lex_consume(ls, ';');
    return vp_decl_type(sp, name, spec);
}

static Aggregate* parse_aggr(LexState* ls);

static AggregateItem parse_aggr_item(LexState* ls)
{
    if(lex_match(ls, TK_struct))
    {
        return (AggregateItem) {
            .kind = AGR_ITEM_SUB,
            .sub = parse_aggr(ls)
        };
    }
    else
    {
        SrcPos sp = lex_srcpos(ls);
        Str** names = NULL;
        do
        {
            Str* name = parse_name(ls);
            vec_push(names, name);
        }
        while(lex_match(ls, ','));
        lex_consume(ls, ':');
        TypeSpec* spec = parse_type(ls);
        lex_consume(ls, ';');
        return (AggregateItem) {
            .sp = sp,
            .kind = AGR_ITEM_FIELD,
            .names = names,
            .type = spec
        };
    }
}

/* Parse struct/union item */
static Aggregate* parse_aggr(LexState* ls)
{
    lex_consume(ls, '{');
    SrcPos sp = lex_srcpos(ls);
    AggregateItem* items = NULL;
    while(!lex_check(ls, '}') && !lex_check(ls, TK_eof))
    {
        AggregateItem item = parse_aggr_item(ls);
        vec_push(items, item);
    }
    lex_consume(ls, '}');
    return vp_aggr_new(sp, AGR_STRUCT, items);
}

/* Parse 'struct' declaration */
static Decl* parse_struct(LexState* ls)
{
    vp_lex_next(ls);    /* Skip 'struct' */
    SrcPos sp = lex_srcpos(ls);
    lex_consume(ls, TK_name);
    Str* name = ls->val.name;
    Aggregate* aggr = parse_aggr(ls);
    return vp_decl_aggr(sp, DECL_STRUCT, name, aggr);
}

/* Parse 'fn' declaration */
static Decl* parse_fn(LexState* ls)
{
    vp_lex_next(ls);    /* Skip 'fn' */
    SrcPos sp = lex_srcpos(ls);
    lex_consume(ls, TK_name);
    Str* name = ls->val.name;

    Param* params = parse_params(ls);
    lex_consume(ls, ':');
    TypeSpec* ret = parse_type(ls);

    Decl* decl = vp_decl_fn(sp, ret, name);
    decl->fn.body = parse_block(ls);
    decl->fn.params = params;

    return decl;
}

/* Parse 'var' */
static Decl* parse_var(LexState* ls)
{
    vp_lex_next(ls);    /* Skip 'var' */
    SrcPos sp = lex_srcpos(ls);
    lex_consume(ls, TK_name);
    Str* name = ls->val.name;

    TypeSpec* spec = NULL;
    if(lex_match(ls, ':'))
        spec = parse_type(ls);

    Expr* e = lex_match(ls, '=') ? expr(ls) : NULL;
    lex_consume(ls, ';');

    return vp_decl_var(sp, name, spec, e);
}

/* Parse simple statement */
static Stmt* parse_stmt_simple(LexState* ls)
{
    Expr* e = expr(ls);
    Stmt* st;
    if(lex_match(ls, '='))
    {
        st = vp_stmt_assign(e->sp, e, expr(ls));
    }
    else
    {
        st = vp_stmt_expr(e->sp, e);
    }
    lex_consume(ls, ';');
    return st;
}

/* Parse a statement */
static Stmt* parse_stmt(LexState* ls)
{
    Stmt* st;
    switch(ls->curr)
    {
        case TK_return:
            st = parse_return(ls);
            break;
        case '{':
            st = parse_block(ls);
            break;
        default:
            st = parse_stmt_simple(ls);
            break;
    }
    return st;
}

/* Parse a declaration */
static Decl* parse_decl(LexState* ls)
{
    Decl* d = NULL;
    switch(ls->curr)
    {
        case TK_fn:
            d = parse_fn(ls);
            break;
        case TK_var:
            d = parse_var(ls);
            break;
        case TK_type:
            d = parse_typedef(ls);
            break;
        case TK_struct:
            d = parse_struct(ls);
            break;
        default:
            break;
    }
    return d;
}

Decl** vp_parse(VpState* V, LexState* ls)
{
    vp_lex_next(ls);    /* Read the first token into "next" */

    Decl** decls = NULL;
    while(!lex_match(ls, TK_eof))
    {
        Decl* d = parse_decl(ls);
        vec_push(decls, d);
    }
    return decls;
}