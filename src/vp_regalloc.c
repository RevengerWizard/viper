...
VReg* vp_vreg_ku(uint64_t u64, VRegSize vsize)
{
    ...
    VReg* vr = vp_ra_spawn(vsize, VRF_UINT);
    vr->u64 = u64;
    return vr;
}
...
VReg* vp_vreg_ki(int64_t i64, VRegSize vsize)
{
    ...
    VReg* vr = vp_ra_spawn(vsize, VRF_INT);
    vr->i64 = i64;
    return vr;
}
...
VReg* vp_vreg_kf(double n, VRegSize vsize)
{
    ...
    VReg* vr = vp_ra_spawn(vsize, VRF_NUM);
    vr->n = n;
    return vr;
}
...
typedef struct
{
    ...
    uint32_t phystemp;  /* # of volatile registers */
...
static void live_occupy(RegAlloc* ra, LiveInterval** actives, uint64_t ioccupy, uint64_t foccupy)
{
    for(uint32_t k = 0; k < vec_len(actives); k++)
    {
        ...
        li->regbits |= (vr->flag & VRF_NUM) ? foccupy : ioccupy;
    }
}
...
static void live_detect(RegAlloc* ra, BB** bbs, uint32_t vreglen, LiveInterval** sorted)
{
    ...
    for(uint32_t i = 0; i < vreglen; i++)
    {
        ...
        if(li->end < 0)
            continue;
        LiveInterval** vec = li->start < 0 ? actives : inactives;
        vec_push(vec, li);
    }
    ...
    for(uint32_t i = 0; i < vec_len(bbs); i++)
    {
        ...
        for(uint32_t j = 0; j < vec_len(bb->irs); j++, nip++)
        {
            ...
            for(uint32_t k = 0; k < 3; k++)
            {
                ...
                if(li->start == ((uint32_t)~1) && !(vr->flag & VRF_PARAM))
                    li->start = nip;
                if(li->end < nip)
                    li->end = nip;
            }
...
static void ra_scan(RegAlloc* ra, LiveInterval** sorted, uint32_t vreglen)
{
    ...
    for(uint32_t i = 0; i < vreglen; i++)
    {
        ...
        if(ip)
        {
            ...
            if(ip && !(occupied & 1ULL << ip))
            {
                regno = ip;
            }
            else
            {
                startidx = prsp->phystemp;
            }
        }
        if(regno < 0)
        {
            for(uint32_t j = startidx; j < prsp->physmax; j++)
            {
                if(!(occupied & (1ULL << j)))
                {
                    regno = j;
